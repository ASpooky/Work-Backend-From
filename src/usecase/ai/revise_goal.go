...
- 会話の中でタスクの頻度・内容・期間の変更や、新しいタスクの追加が明確に求められた場合は、
  remove_task_ids(削除する既存タスクのID)とnew_tasks(追加する新しいタスク)を使ってください。
- 見直し候補タスクがまだタスクが無くても（「候補タスク: なし」の場合でも）、会話の中で
  具体的な日々のタスク内容についてユーザーと合意できていれば、new_tasksとしてそれを提案して
  ください。「既存タスクの変更」だけでなく「まだ無いタスクの新規追加」も対象です。
- 目標の期限や達成条件だけの変更で、タスク自体の話が出ていない場合は、
